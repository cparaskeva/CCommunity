...
/**
 * bp_alerts_screen_notification_settings()
 *
 ...
 */
function bp_alerts_screen_notification_settings() {
	global $current_user;
	...
	<table class="notification-settings" id="bp-example-notification-settings">
		...
		<?php do_action( 'bp_alerts_notification_settings' ); ?>

		</tbody>
	</table>
<?php
}
//add_action( 'bp_notification_settings', 'bp_alerts_screen_notification_settings' );


/**
 * bp_alerts_remove_screen_notifications()
 *
 * Remove a screen notification for a user.
 */
function bp_alerts_remove_screen_notifications() {
	global $bp;
	 ...
	 * The following command will do so.
 	 */
	bp_core_delete_notifications_for_user_by_type( $bp->loggedin_user->id, $bp->alerts->slug, 'new_high_five' );
}
add_action( 'bp_alerts_screen_one', 'bp_alerts_remove_screen_notifications' );
add_action( 'xprofile_screen_display_profile', 'bp_alerts_remove_screen_notifications' );


/**
 * bp_alerts_format_notifications()
 *
 ...
 * examples of usage.
 */
function bp_alerts_format_notifications( $action, $item_id, $secondary_item_id, $total_items ) {
	global $bp;
	...
	switch ( $action ) {
		case 'new_high_five':
			...
			if ( (int)$total_items > 1 ) {
				return apply_filters( 'bp_alerts_multiple_new_high_five_notification', '<a href="' . $bp->loggedin_user->domain . $bp->alerts->slug . '/screen-one/" title="' . __( 'Multiple high-fives', 'bp-example' ) . '">' . sprintf( __( '%d new high-fives, multi-five!', 'bp-example' ), (int)$total_items ) . '</a>', $total_items );
			} else {
				$user_fullname = bp_core_get_user_displayname( $item_id, false );
				$user_url = bp_core_get_user_domain( $item_id );
				return apply_filters( 'bp_alerts_single_new_high_five_notification', '<a href="' . $user_url . '?new" title="' . $user_fullname .'\'s profile">' . sprintf( __( '%s sent you a high-five!', 'bp-example' ), $user_fullname ) . '</a>', $user_fullname );
			}
		break;
	}

	do_action( 'bp_alerts_format_notifications', $action, $item_id, $secondary_item_id, $total_items );
...
function bp_alerts_send_high_five_notification( $to_user_id, $from_user_id ) {
	global $bp;
	...
	$sender_highfive_link = site_url( BP_MEMBERS_SLUG . '/' . $sender_ud->user_login . '/' . $bp->alerts->slug . '/screen-one' );
...
add_action( 'bp_alerts_send_high_five', 'bp_alerts_send_high_five_notification', 10, 2 );
