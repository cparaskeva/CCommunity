...
/**
 * bp_patents_licenses_screen_notification_settings()
 *
 ...
 */
function bp_patents_licenses_screen_notification_settings() {
	global $current_user;
	...
	<table class="notification-settings" id="bp-example-notification-settings">
		...
		<?php do_action( 'bp_patents_licenses_notification_settings' ); ?>

		</tbody>
	</table>
<?php
}
//add_action( 'bp_notification_settings', 'bp_patents_licenses_screen_notification_settings' );


/**
 * bp_patents_licenses_remove_screen_notifications()
 *
 * Remove a screen notification for a user.
 */
function bp_patents_licenses_remove_screen_notifications() {
	global $bp;
	 ...
	 * The following command will do so.
 	 */
	bp_core_delete_notifications_for_user_by_type( $bp->loggedin_user->id, $bp->patents_licenses->slug, 'new_high_five' );
}
add_action( 'bp_patents_licenses_screen_one', 'bp_patents_licenses_remove_screen_notifications' );
add_action( 'xprofile_screen_display_profile', 'bp_patents_licenses_remove_screen_notifications' );


/**
 * bp_patents_licenses_format_notifications()
 *
 ...
 * examples of usage.
 */
function bp_patents_licenses_format_notifications( $action, $item_id, $secondary_item_id, $total_items ) {
	global $bp;
	...
	switch ( $action ) {
		case 'new_high_five':
			...
			if ( (int)$total_items > 1 ) {
				return apply_filters( 'bp_patents_licenses_multiple_new_high_five_notification', '<a href="' . $bp->loggedin_user->domain . $bp->patents_licenses->slug . '/screen-one/" title="' . __( 'Multiple high-fives', 'bp-example' ) . '">' . sprintf( __( '%d new high-fives, multi-five!', 'bp-example' ), (int)$total_items ) . '</a>', $total_items );
			} else {
				$user_fullname = bp_core_get_user_displayname( $item_id, false );
				$user_url = bp_core_get_user_domain( $item_id );
				return apply_filters( 'bp_patents_licenses_single_new_high_five_notification', '<a href="' . $user_url . '?new" title="' . $user_fullname .'\'s profile">' . sprintf( __( '%s sent you a high-five!', 'bp-example' ), $user_fullname ) . '</a>', $user_fullname );
			}
		break;
	}

	do_action( 'bp_patents_licenses_format_notifications', $action, $item_id, $secondary_item_id, $total_items );
...
function bp_patents_licenses_send_high_five_notification( $to_user_id, $from_user_id ) {
	global $bp;
	...
	$sender_highfive_link = site_url( BP_MEMBERS_SLUG . '/' . $sender_ud->user_login . '/' . $bp->patents_licenses->slug . '/screen-one' );
...
add_action( 'bp_patents_licenses_send_high_five', 'bp_patents_licenses_send_high_five_notification', 10, 2 );
