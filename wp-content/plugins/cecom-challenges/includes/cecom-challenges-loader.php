 ...
 * @since 1.6
 */
class BP_Challenges_Component extends BP_Component {
    ...
    function __construct() {
        ...
        parent::start(
                'challenges', __('Challenges', 'cecom-challenges'), BP_CHALLENGE_PLUGIN_DIR
        );
     ...
     *       setup):
     *           /wp-content/plugins/cecom-challenges/notifications.php
     *           /wp-content/plugins/cecom-challenges/filters.php
     ...
     *           /wp-content/plugins/cecom-challenges/cecom-challenges/notifications.php
     *           /wp-content/plugins/cecom-challenges/cecom-challenges/filters.php
     ...
     *           /wp-content/plugins/cecom-challenges/cecom-challenges/cecom-challenges-notifications.php
     *           /wp-content/plugins/cecom-challenges/cecom-challenges/cecom-challenges-filters.php
     *
     ...
     * 			    bp_has_challenges()), those functions should go in this file.
    ...
    function includes() {
        ...
        // Files to include
        $includes = array(
            'includes/cecom-challenges-screens.php',
            'includes/cecom-challenges-filters.php',
            'includes/cecom-challenges-classes.php',
            'includes/cecom-challenges-activity.php',
            'includes/cecom-challenges-template.php',
            'includes/cecom-challenges-functions.php',
            'includes/cecom-challenges-notifications.php',
            'includes/cecom-challenges-cssjs.php',
        );
    ...
    function setup_globals() {
        ...
        if (!defined('BP_CHALLENGE_SLUG'))
            define('BP_CHALLENGE_SLUG', $this->id);
        ...
        $global_tables = array(
            'table_name' => 'ext_challenge' //$bp->table_prefix . 'bp_challenges'
        );

        // Set up the $globals array to be passed along to parent::setup_globals()
        $globals = array(
            'slug' => BP_CHALLENGE_SLUG,
        ...
        //Set the subdomain for each challenge e.g. /cecommunity/challenges/{subdomain}{challengeID}
        $bp->{$this->id}->challenges_subdomain = 'challenge';

        /** Single Patent_License Globals ********************************************* */
        // Are we viewing a single challenge?
        if (bp_is_challenge_component() && $challenge_id = BP_Challenge::challenge_exists(bp_current_action())) {
            $bp->is_single_item = true;
            $current_challenge_class = apply_filters('bp_challenges_current_challenge_class', 'BP_Challenge');
            $this->current_challenge = apply_filters('bp_challenges_current_challenge_object', new $current_challenge_class(Array('id' => $challenge_id)));
            $this->current_challenge->slug = $bp->{$this->id}->challenges_subdomain . $challenge_id;

            // When in a single challenge, the first action is bumped down one because of the
            // challenge name, so we need to adjust this and set the group name to current_item.
            $bp->current_item = bp_current_action();
            $bp->current_action = bp_action_variable(0);
            array_shift($bp->action_variables);

            //Set if the user is owner of the challenge
            bp_update_is_item_admin(bp_loggedin_user_id() == $this->current_challenge->uid, 'challenges');

            //echo "Current Action: ".$bp->current_action." Patent_LicenseID: ".$challenge_id. "   ". $bp->is_single_item. "Single item? ". bp_is_single_item() ; //die();
        } else {
     ...
     */
    function setup_nav() {
        // Add 'Challenges' to the main navigation
        $main_nav = array(
            'name' => sprintf(__('Challenges  <span>%s</span>', 'buddypress'), bp_get_total_challenges_count_for_user()),
            'slug' => bp_get_challenges_slug(),
            'position' => 80,
            'screen_function' => 'bp_challenges_screen_one',
            'default_subnav_slug' => 'screen-one'
        );

        $challenge_link = trailingslashit(bp_loggedin_user_domain() . bp_get_challenges_slug());

        // Add a few subnav items under the main Example tab
        $sub_nav[] = array(
            'name' => __('Published Challenges', 'cecom-challenges'),
            'slug' => 'screen-one',
            'parent_url' => $challenge_link,
            'parent_slug' => bp_get_challenges_slug(),
            'screen_function' => 'bp_challenges_screen_one',
            'position' => 10
        );
        ...
        $sub_nav[] = array(
            'name' => __('Create a challenge', 'cecom-challenges'),
            'slug' => 'create-challenge',
            'parent_url' => $challenge_link,
            'parent_slug' => bp_get_challenges_slug(),
            'screen_function' => 'bp_challenges_create_challenge',
            'position' => 20
        );
        ...
        if (bp_is_challenge_component() && bp_is_single_item()) {
            ...
            $main_nav = array(
                ...
                'slug' => $this->current_challenge->slug,
                'position' => -1, // Do not show in BuddyBar
                'screen_function' => 'challenges_screen_challenge_home',
                'default_subnav_slug' => $this->default_extension,
                'item_css_id' => $this->id
            );

            $challenge_link = trailingslashit(bp_get_root_domain() . '/' . bp_get_challenges_root_slug() . '/' . $this->current_challenge->slug . '/');

            // If the user is challenge ownner, then show the challenge admin nav item
            if (bp_is_item_admin()) {
                global $bp;
                $sub_nav[] = array(
                    ...
                    'slug' => 'admin',
                    'parent_url' => $challenge_link,
                    'parent_slug' => $this->current_challenge->slug,
                    'screen_function' => 'challenges_screen_challenge_admin',
                    'position' => 40,
        ...
        if (isset($this->current_challenge->user_has_access)) {
            do_action('challenges_setup_nav', $this->current_challenge->user_has_access);
        } else {
            do_action('challenges_setup_nav');
        }
    }
 ...
 *   function bp_challenges_var_dump() {
 *   	  global $bp;
 * 	  var_dump( $bp->example );
 *   }
 *   add_action( 'bp_init', 'bp_challenges_var_dump' );
 * It goes without saying that you should not do this on a production site!
 *
 * @package Challenges Component
 */
function bp_challenges_load_core_component() {
    global $bp;

    $bp->challenges = new BP_Challenges_Component();
}

add_action('bp_loaded', 'bp_challenges_load_core_component', 1);
?>