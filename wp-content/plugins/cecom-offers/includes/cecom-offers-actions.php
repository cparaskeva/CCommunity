 ...
 * bp_is_offer_component(), along with the bp_is_current_action() and bp_is_action_variable()
 ...
 * @since 1.6
 */
function bp_offers_high_five_save() {

	if ( bp_is_offer_component() && bp_is_current_action( 'screen-one' ) && bp_is_action_variable( 'send-h5', 0 ) ) {
		...
		if ( bp_is_my_profile() ) {
			...
			if ( bp_offers_send_highfive( bp_displayed_user_id(), bp_loggedin_user_id() ) )
				bp_core_add_message( __( 'High-five sent!', 'bp-example' ) );
			else
				bp_core_add_message( __( 'High-five could not be sent.', 'bp-example' ), 'error' );
		}

		bp_core_redirect( bp_displayed_user_domain() . bp_get_offers_slug() . '/screen-one' );
	}
}
add_action( 'bp_actions', 'bp_offers_high_five_save' );
